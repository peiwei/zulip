...
def list_to_streams(streams_raw, user_profile, autocreate=False, invite_only=False):
    ...
    stream_set = set(stream_name.strip() for stream_name in streams_raw)  # type: Set[str]
    rejects = []
    ...
    existing_stream_map = bulk_get_streams(user_profile.realm, stream_set)  # type: Dict[str, Stream]
    ...
    if rejects:
        ...
        for stream_name in rejects:
            stream, created = create_stream_if_needed(user_profile.realm,  # type: Stream, bool
                                                      stream_name,
...
def principal_to_user_profile(agent, principal):
    ...
    principal_doesnt_exist = False  # type: bool
    try:
        principal_user_profile = get_user_profile_by_email(principal)  # type: UserProfile
...
def add_subscriptions_backend(request, user_profile,
    ...
    # type: (HttpRequest, UserProfile, str, List[Dict[str, str]], bool, bool, Optional[List[str]], bool) -> HttpResponse
